...
if (isset($_GET['edita_receita'])) {
    ...
    $nome_receita = $_GET['edita_receita'];
    ...
    if (mysqli_num_rows($resultado) > 0) {
        ...
        echo        '<form action="PHP/EditaReceita.php" method="POST" class="cadastro-form" enctype="multipart/form-data" >';
        echo            '<input type="hidden" name="foto_atual" value="' . $row['Endereco_receita'] . '">';

        echo            '<fieldset> <!-- INSERE AS INFORMAÇÕES DA RECEITA -->';
        echo                '<label for="nome_receita">Nome da receita:</label>';
        echo                '<input type="text" id="nome_receita" name="nome_receita" value="' . $row["nome_receita"] . '">'; // Corrigido: Adiciona o valor do campo nome_receita

        echo                '<div class="procurafoto">';
        echo                    '<label for="foto" class="perfil">';
        echo                        '<img src="./img/' . $row["Endereco_receita"] . '" alt="Imagem de Perfil" id="imagem">';
        echo                        '<figcaption>Altere a sua foto aqui</figcaption>';
        echo                    '</label>';
        echo                    '<input type="file" name="foto" id="foto" onchange="preview();">';
        echo                '</div>';
        ...
        echo                    '<textarea style="width: 77%; height:51%;" id="resumo_receita" name="resumo_receita" rows="8">' . $row["resumo_receita"] . '</textarea>';
        echo                    '<br><br><label for="ingredientes_receita">Ingredientes:</label>';
        echo                    '<textarea id="ingredientes_receita" name="ingredientes_receita" rows="8">' . $row["ingredientes_receita"] . '</textarea>';
        echo                    '<br><br><label for="modopreparo_receita">Modo de Preparo:</label>';
        echo                    '<textarea id="modopreparo_receita" name="modopreparo_receita" rows="8">' . $row["modopreparo_receita"] . '</textarea>';
        echo                '</div>';
        echo            '</fieldset>';
        echo            '<a href="MeuEspaco.php" class="button">Voltar minha página</a>';
        echo            '<button class="button "type="submit">Salvar modificações</button>';
        echo        '</form>';
        echo    '</div>';
        echo '</section>';
    }
